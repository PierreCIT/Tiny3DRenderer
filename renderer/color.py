...
class Color:
    ...
    @staticmethod
    @jit
    def _interpolate_numpy(c_array: np.ndarray, w: np.ndarray, ignore_alpha: bool = True) -> np.ndarray:
        """Interpolate colors based on weights (must be same size)"""
        n_w = w / w.sum()
        r = (n_w * c_array.T).T.sum(axis=0)
        if ignore_alpha:
    ...
    @staticmethod
    def interpolate_numpy(colors: list['Color'], w: np.ndarray, ignore_alpha: bool = True) -> np.ndarray:
        return Color._interpolate_numpy(np.asarray([c.to_array() for c in colors], dtype=np.uint8), w, ignore_alpha)
